...
public class LocalGlobalVisitor implements Visitor {
    ...
    public void visit(FormalParameter expr) {
        if (expr.ct.getType() == Type.VOID) {
            throw new LocalGlobalException(expr, "No parameter may have a “type” of void.");
        }
        if (paramNames.contains(expr.id.value)) {
            throw new LocalGlobalException(expr, "No two parameters of a function may have the same name");
        }
    ...
    public void visit(Identifier expr) {
        if (!varName.contains(expr.value) && !paramNames.contains(expr.value) && !funcNames.contains(expr.value)) {
            throw new LocalGlobalException(expr, "Each local variable must be defined before being used.");
        }
    }
    ...
    public void visit(Program expr) {
        if (expr.functionList.isEmpty()) {
            throw new LocalGlobalException(expr, "A program must contain at least one function");
        }
        for (Function f : expr.functionList) {
            if (funcNames.contains(f.funcDecl.id.value)) {
                throw new LocalGlobalException(expr, "No two functions may have the same name");
            }
        ...
        if (!hasMain) {
            throw new LocalGlobalException(expr, "One function must be called main, and "
    ...
    public void visit(VariableDeclaration expr) throws LocalGlobalException {
        if (expr.ct.getType() == Type.VOID) {
            throw new LocalGlobalException(expr, "No local variable may have a “type” of void.");
        }
        if (varName.contains(expr.id.value)) {
            throw new LocalGlobalException(expr, " No two local variables declared in a function may have the same name.");
        }
        if (paramNames.contains(expr.id.value)) {
            throw new LocalGlobalException(expr, "A local variable may not hide the name of a parameter.");
        }
